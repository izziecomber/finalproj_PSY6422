...
The data source I have chose provided this statistic:
...
As a result I decided to explore the trends of sexuality. 
...
### Data source
The raw data was obtained from the Office of National Statistics, from the Annual Population Survey (APS). 
*Office for National Statistics (ONS), released 27 September 2023, ONS website, statistical bulletin, Sexual orientation, [UK: 2021 and 2022](https://www.ons.gov.uk/peoplepopulationandcommunity/culturalidentity/sexuality/bulletins/sexualidentityuk/2021and2022#comparing-aps-estimates-with-census-2021)*

**Some notes on the data (as noted by the Office of National Statistics):**
*Extracted directly from the [Quality and methodology information](https://www.ons.gov.uk/peoplepopulationandcommunity/culturalidentity/sexuality/methodologies/sexualidentityukqmi#methods-used-to-produce-the-sexual-orientation-data)*
...
```{r loadlibrary, echo = TRUE, results = 'hide', warning = FALSE, message = FALSE}
...
## Sanity checks
```{r expectations, message=FALSE, warning=FALSE, results = 'hide'}
...
The first visualisation of the identities for each region was helpful to see the data trends across the UK but it was very cluttered and busy. For this plot I decided to use only the total UK estimate data to get feel for the general trend of the population.
```{r barchart, results = 'hide'}
...
```{r lineplot, results='hide', message=FALSE, warning=FALSE}
...
```{r specific_sex, results='hide', message=FALSE, warning=FALSE}
...
```{r improved, results='hide', message=FALSE, warning=FALSE}
...
dynamic_colors <- colorRampPalette(RColorBrewer::brewer.pal(8, "Set2"))(length(unique(aggregated_data$Region)))
